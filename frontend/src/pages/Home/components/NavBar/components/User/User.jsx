...
import { deleteDialog, fetchDialogsData } from '../../../../../../redux/actions/dialogs';

const User = ({ userId, fullName, avatar, isOnline, dialogId, selectedDialog, time }) => {
	...
	return (
		<div>
			<Link to={'/dialog/' + dialogId} className={classes.Link} onClick={handleOnClick}>
				<ListItem button key={userId} selected={dialogId === selectedDialog ? true : false}>
					<ListItemIcon>
					...
					<MoreButton
						...
						options={['Delete']}
						onDelete={(event) => {
							dispatch(deleteDialog(dialogId)).then(() => {
